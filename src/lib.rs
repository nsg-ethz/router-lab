...
//! `router-lab/config` folder. The most important aspect is changing the SSH host names to match
...
//! at once. Thus, the main structure `RouterLab` contains a type parameter `S` which is either
...
//!
//! To enforce that only a single instance can actively use the lab, the `RouterLab` will create a
//! lock file on the server called `/tmp/router-lab.lock`. When creating the lock, it will write the
//! user into the file (for debugging purpose). When the lock already exists, the `RouterLab` cannot
//! be turned from `Inactive` to `Active`. See [`RouterLab`] for more details.
//!
//! # Experiment Setup
//!
//! ## The Physical Setup
...
//!
//! To emulate events that are changing, you can configure `RouterLab` to change the advertisements
//! during the experiment. To do this, you need to call [`RouterLab::step_external_time`] to
//! increment a counter, followed by either calling [`RouterLab::advertise_route`] or
//! [`RouterLab::withdraw_route`]. The generated python script to interact with ExaBGP will then
...
use hardware_mapping::HardwareMapping;
use ipnet::Ipv4Net;
...
use bgpsim::{
    export::{Addressor, CiscoFrrCfgGen, DefaultAddressor, DefaultAddressorBuilder, ExportError},
    prelude::*,
    types::NonOverlappingPrefix,
};

use config::{RouterProperties, CONFIG};
use server::ExternalRouterHandle;

/// The RouterLab is in offline mode. This means that it will not do anything on the physical
/// hardware, but you can still generate the configuration strings.
pub struct Inactive;

/// The `RouterLab` is connected to the physical hardware, and actively managing it. The structure
/// contains the established sessions. There can always be at most one `RouterLab<'n, Q, Active>`
...
/// always be at most one `RouterLab<'n, Q, Active>` instance. This is enforced by creating a lock
/// file on the server. A `RouterLab<'n, Q, Inactive>` instance will not connect itself to any of the
/// physical devices in the lab, and can only be used to generate the configuration strings. The
/// lock file is located on the server at `/tmp/router-lab.lock`, and it will contain the username of
/// the user that has created the lock.
///
/// Calling [`RouterLab::new`] will create an inactive instance. This inactive instance can be used
/// to generate the configuration strings, but it will not connect itself to any device via
/// SSH. Calling [`RouterLab::connect`] on a `RouterLab<'n, Q, Inactive>` will try to connect itself
/// to all devices, returning a `RouterLab<'n, Q, Active>` instance.
pub struct RouterLab<'n, P: Prefix, Q, Ospf: OspfImpl, S = Inactive> {
    ...
    external_routers: BTreeMap<RouterId, ExternalRouterHandle<P>>,
    link_delays: HashMap<(RouterId, RouterId), u32>,
    hardware_mapping: HardwareMapping,
    state: S,
}

impl<'n, P: Prefix, Q: EventQueue<P> + Clone, Ospf: OspfImpl> RouterLab<'n, P, Q, Ospf, Inactive> {
    ...
    pub fn new(net: &'n Network<P, Q, Ospf>) -> Result<Self, RouterLabError> {
        let empty_binding = HashMap::<_, &str>::new();
        Self::with_bindings(net, &empty_binding, &empty_binding)
    }

    /// Generate a new instance to manage the network by manually assigning internal/external
    /// routers to VDCs. If an external router is assigned to a physical device, it is configured
    /// with an exabgp session to advertise all available routes and can be controlled via the
    /// `router_lab.external_routers`
    ///
    /// This function will not yet connect to any router. Any internal router not explicitly
    /// mentioned by `internal_binding` will be assigned automatically. An error is thrown if a
    /// router has not enough interfaces.
    pub fn with_bindings<S: AsRef<str>>(
        net: &'n Network<P, Q, Ospf>,
        internal_binding: &HashMap<RouterId, S>,
        external_binding: &HashMap<RouterId, S>,
    ) -> Result<Self, RouterLabError> {
        let routers = Self::prepare_internal_routers(net, internal_binding, external_binding)?;
        let external_routers = Self::prepare_external_routers(net, external_binding)?;
    ...
    /// them properly. This function will create the lock on the server. Dropping `RouterLab<'n, Q,
    /// Active>` will also drop the [`server::ServerSession`], which will automatically release the
    /// lock.
    pub async fn connect(mut self) -> Result<RouterLab<'n, P, Q, Ospf, Active>, RouterLabError> {
        ...
        // generate and log the hardware mapping
        lab.compute_hardware_mapping()?;

        Ok(lab)
    }
}

impl<'n, P: Prefix + NonOverlappingPrefix, Q, Ospf: OspfImpl> RouterLab<'n, P, Q, Ospf, Inactive> {
...
impl<'n, P: Prefix, Q, Ospf: OspfImpl> RouterLab<'n, P, Q, Ospf, Inactive> {
    ...
    pub fn addressor_mut(&mut self) -> &mut DefaultAddressor<'n, P, Q, Ospf> {
        &mut self.addressor
    }
}
...
impl<'n, P: Prefix, Q, Ospf: OspfImpl> RouterLab<'n, P, Q, Ospf, Active> {
    ...
    pub async fn disconnect(self) -> Result<RouterLab<'n, P, Q, Ospf, Inactive>, RouterLabError> {
...
impl<'n, P: Prefix, Q, Ospf: OspfImpl, S> RouterLab<'n, P, Q, Ospf, S> {
    ...
    /// provided.
    pub fn addressor(&self) -> &DefaultAddressor<'n, P, Q, Ospf> {
        &self.addressor
    }

    /// Get a reference to the network
    pub fn net(&self) -> &'n Network<P, Q, Ospf> {
        self.net
    }

    /// Get a reference to the routers
    pub fn routers(&self) -> &BTreeMap<RouterId, (&'static RouterProperties, CiscoFrrCfgGen<P>)> {
        &self.routers
    }

    /// Get a reference to the external routers
    pub fn external_routers(&self) -> &BTreeMap<RouterId, ExternalRouterHandle<P>> {
...
pub enum RouterLabError {
    ...
    WrongSupervisorStatus(String),
    /// The given VDC name was not found
    #[error("Unknown VDC name {0}")]
    UnknownVdc(String),
}
