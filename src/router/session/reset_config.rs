...
transition! {
 // Label,          State,     Regex on the line             => next state
    ...
    (IF_IP,         Interface, r"^ip")                       => Negate,
    (IF_MAC_ADDR,   Interface, r"^mac-address ")             => Negate,
    (IF_NO_SHUT,    Interface, r"^no shutdown")              => Negate,
    (IF_SHUT,       Interface, r"^shutdown")                 => Ignore,

    (OSPF_BLOCK,    Root,      r"^router ospf")              => Enter(Ospf),
    (OSPF_ELEMENT,  Ospf,      r".*")                        => Negate,

    (BGP_BLOCK,     Root,      r"^router bgp")               => Negate,
    (BGP_ELEMENT,   Bgp,       r".*")                        => Negate,
}

/// Create a set of commands that will invert the configuration on a device. Configuration that is
/// inverted is only the configuration that is set by `bgpsim` and `router_lab`. Any other
