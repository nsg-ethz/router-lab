...
impl CiscoSession {
    ...
    pub async fn new_with_reset(destination: impl Into<String>) -> Result<Self, RouterLabError> {
        // First, create the session
        let s = Self::new(destination).await?;

        // then, reset the configuration
        let mut shell = s.shell();
    ...
    /// # }
    /// ```
    pub fn shell(&self) -> CiscoShell {
        CiscoShell::new(self.name())
    }
