...
use crate::{
    ...
    server::choose_equidistant_k,
    ssh::{control_path, std_command, wait_prompt},
};
...
impl CiscoShell {
    ...
    async fn try_execute<O, F, Fut>(&self, f: F) -> Result<O, CiscoShellError>
    where
        F: FnMut(Shell) -> Fut,
        Fut: Future<Output = Result<O, CiscoShellError>>,
    {
        Shell::try_execute(&self.name, f).await
    }

    /// Get the running configuration
    pub async fn get_running_config(&mut self) -> Result<String, CiscoShellError> {
        self.try_execute(|mut shell| async move { shell.show("running-config").await })
            .await
    }

    /// Get the startup configuration
    pub async fn get_startup_config(&mut self) -> Result<String, CiscoShellError> {
        self.try_execute(|mut shell| async move { shell.show("startup-config").await })
            .await
    }
    ...
    pub async fn in_initial_state(&mut self) -> Result<bool, CiscoShellError> {
        let (startup_config, running_config) = self
            .try_execute(|mut shell| async move {
                let startup_config = shell.show("running-config").await?;
                let running_config = shell.show("startup-config").await?;
                Ok((startup_config, running_config))
            })
            .await?;
    ...
    pub async unsafe fn reset_configuration_hard(&mut self) -> Result<(), CiscoShellError> {
        self.try_execute(|mut shell| async move {
            shell
                .send_cmd_expect("delete bootflash:///startup-config no-prompt", |stdout| {
            ...
            shell
                ...
                .await?;
            shell
                .send_cmd_expect("configure replace bootflash:///startup-config", |stdout| {
    ...
    pub async fn configure(&mut self, conf: impl AsRef<str>) -> Result<(), CiscoShellError> {
        let sleep_time = std::env::var("CISCO_LAB_CONFIGURE_SLEEP_MS")
            .ok()
        ...
        self.try_execute(|mut shell| async move {
            shell
                .send_cmd_expect("configure terminal", str::is_empty)
                .await?;
            for line in conf.lines() {
                ...
                shell
                    .send_cmd_expect(line, |s| check_reconfig_output(line, s))
                    .await?;
                // sleep for an amount of time, configurable by the environment variable
                if let Some(dur) = sleep_time {
                    sleep(dur).await;
                }
            }
            shell.send_cmd_expect("end", str::is_empty).await?;
            Ok(())
        })
        .await?;
    ...
    pub async fn clear_routing(&mut self, net: Ipv4Net) -> Result<(), CiscoShellError> {
        self.try_execute(|mut shell| async move {
            shell
                .send_cmd_expect(&format!("clear routing ipv4 unicast {net}"), |s| {
    ...
    pub async fn bgp_refresh_routes(
        ...
        self.try_execute(|mut shell| async move {
            let command = if let Some(addr) = neighbor {
                format!("clear ip bgp {addr} soft")
            } else {
                String::from("clear ip bgp * soft")
            };
            shell.send_cmd_expect(&command, str::is_empty).await
        })
        .await
    }
    ...
    pub async fn get_ospf_neighbors(&mut self) -> Result<Vec<OspfNeighbor>, CiscoShellError> {
        Ok(OspfNeighbor::from_table(
            &self
                .try_execute(|mut shell| async move { shell.show("ip ospf neighbors").await })
                .await?,
        )?)
    }
    ...
    pub async fn get_ospf_state(&mut self) -> Result<HashMap<Ipv4Net, OspfRoute>, CiscoShellError> {
        Ok(OspfRoute::from_xml_output(
            &self
                .try_execute(|mut shell| async move { shell.show("ip ospf route | xml\n").await })
                .await?,
        )?)
    }
    ...
    pub async fn get_ospf_route(
        ...
        let mut routes = OspfRoute::from_xml_output(
            &self
                .try_execute(|mut shell| async move {
                    shell.show(format!("ip ospf route {net} | xml")).await
                })
                .await?,
        )?;
    ...
    pub async fn get_bgp_neighbors(&mut self) -> Result<Vec<BgpNeighbor>, CiscoShellError> {
        Ok(BgpNeighbor::from_table(
            &self
                .try_execute(|mut shell| async move { shell.show("ip bgp summary").await })
                .await?,
        )?)
    }
    ...
    pub async fn get_bgp_route(
        ...
        Ok(BgpRoute::from_detail(
            self.try_execute(|mut shell| async move {
                shell.show(format!("bgp ipv4 unicast {net} detail")).await
            })
            .await?,
        )?
        .remove(&net))
    ...
    pub async fn get_bgp_routes(
        ...
        Ok(BgpRoute::from_detail(
            self.try_execute(
                |mut shell| async move { shell.show("bgp ipv4 unicast detail").await },
            )
            .await?,
        )?)
    }
    ...
    /// `Self::get_bgp_route` up to 5 times, selecting prefixes equidistantly from the given
    /// networks.
    async fn get_bgp_routes_for_networks(
        ...
        for net in choose_equidistant_k(5, networks.into_iter().collect_vec().into_iter()) {
    ...
    pub async fn get_bgp_routes_matching_route_map(
        ...
        Ok(BgpRoute::from_detail(
            self.try_execute(|mut shell| async move {
                shell
                    .show(format!("bgp ipv4 unicast route-map {rm} detail"))
                    .await
            })
            .await?,
        )?)
    ...
    pub(in super::super) async fn wait_convergence_task(
        ...
        loop {
            ...
            match timeout(Duration::from_secs(1), state_rx.recv()).await {
                ...
                // simply go back to the loop and check the state.
                Err(_) => {}
            }
        }
    }

    /// Check if all bgp next-hops equal to `exp_bgp_routes`. Checks at most 5 prefixes on the
    /// devices themselves.
    pub(crate) async fn check_bgp_next_hop(
        ...
        for (net, next_hop) in
            choose_equidistant_k(5, exp_bgp_routes.iter().collect_vec().into_iter())
        {
...
/// Private structure that actually does the interaction.
struct Shell {
    name: String,
    _child: Child,
    stdout: ChildStdout,
...
impl Shell {
    /// Number of times to retry before throwing an error. Setting this to 0 will disable the
    /// retry-feature. In total, we will attempt to execute the operation `MAX_RETRY + 1` times.
    const MAX_RETRY: usize = 2;

    /// Execute a command, while re-connecting to the shell if there is an error.
    async fn try_execute<O, F, Fut>(name: &str, mut f: F) -> Result<O, CiscoShellError>
    where
        F: FnMut(Shell) -> Fut,
        Fut: Future<Output = Result<O, CiscoShellError>>,
    {
        let mut i = 0;
        loop {
            let result = match Self::new(name).await {
                Ok(shell) => f(shell).await,
                Err(e) => Err(e),
            };
            // create the shell
            match result {
                Ok(o) => return Ok(o),
                // If an error occurred, but we have already retried too many times
                Err(e) if i >= Self::MAX_RETRY => {
                    log::warn!("[{name}] Operation failed: {e}");
                    log::error!("[{name}] Operation failed after {i} attempts!");
                    return Err(e);
                }
                Err(e) => match e {
                    CiscoShellError::IoError(e) => {
                        match e.kind() {
                            ...
                            | ErrorKind::TimedOut => {
                                // retry
                                log::warn!("[{name}] Operation failed: {e}");
                                // delete the control_file that corresponds to that process
                                tokio::fs::remove_file(control_path(name)).await?;
                                // wait for 3 seconds
                                sleep(Duration::from_secs(3)).await;
                                // retry the command
                            }
                            _ => return Err(CiscoShellError::IoError(e)),
                        }
                    }
                    e => return Err(e),
                },
    ...
    /// `cmd.stdout(piped())`, `cmd.stdin(piped())`, `cmd.stderr(piped())`, and
    /// `cmd.kill_on_drop(true)`.
    async fn new(name: &str) -> Result<Self, CiscoShellError> {
        let mut child = Command::from(std_command(name, crate::ssh::EMPTY))
            .kill_on_drop(true)
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .stdin(Stdio::piped())
...
/// Check if the output of a reconfiguration command is ok, or whether we should throw an error.
fn check_reconfig_output(cmd: &str, output: &str) -> bool {
    if output.is_empty() {
        return true;
    } else if cmd.trim().starts_with("no") {
        // if we remove something, we allow that the output sais the thing does not exist!
        if output.starts_with('%')
            && (output.contains("does not exist") || output.contains("doesn't exist"))
        {
            return true;
        }
    }
