...
use super::{table_parser::parse_table_non_empty, ParseError};
...
impl BgpNeighbor {
    ...
    pub fn from_table(table: &str) -> Result<Vec<BgpNeighbor>, ParseError> {
        ...
        let empty_line = match table.find("\n\n") {
            Some(pos) => pos,
            // if table is empty, then there is no new line here!
        ...
        let mut result = Vec::new();
        for row in parse_table_non_empty(table, fields)? {
...
mod test {
    ...
    #[test]
    fn parse_bgp_neighbors() {
        let table = "BGP summary information for VRF default, address family IPv4 Unicast
BGP router identifier 1.0.4.1, local AS number 65535
BGP table version is 1003179813, IPv4 Unicast config peers 4, capable peers 1
0 network entries and 0 paths using 0 bytes of memory
BGP attribute entries [0/0], BGP AS path entries [0/0]
BGP community entries [0/0], BGP clusterlist entries [0/0]

Neighbor        V    AS    MsgRcvd    MsgSent   TblVer  InQ OutQ Up/Down  State/PfxRcd
1.0.0.1         4 65535          0        265        0    0    0 05:25:12 Idle     
1.0.1.1         4 65535          0        264        0    0    0 05:25:11 Idle     
1.0.2.1         4 65535          0        261        0    0    0 05:25:09 Idle     
1.0.3.1         4 65535     201616        330 1003179813    0    0 05:25:04 0         
";
        assert_eq!(
            BgpNeighbor::from_table(table).unwrap(),
            vec![
                BgpNeighbor {
                    id: "1.0.0.1".parse().unwrap(),
                    as_id: AsId(65535),
                    connected: false,
                    routes_received: 0,
                },
