...
impl TrafficCaptureHandle {
    ...
    pub async fn start(&mut self, frequency: u64) -> Result<(), TrafficCaptureError> {
        ...
        let cmd = if !CONFIG.server.disable_collector {
            format!("sudo collector -b 256 {}", &CONFIG.server.exabgp_iface)
        } else {
            "echo \"Collector has been disabled, not producing any output.\"".to_string()
        };
        ...
        self.collector_stdout = if !CONFIG.server.disable_collector {
            Some(collector_stdout)
        } else {
            None
        };
...
pub enum TrafficCaptureError {
    ...
    /// Cannot parse number of dropped packets
    #[error("Cannot parse number of dropped packets: {0}")]
