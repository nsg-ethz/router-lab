...
const LOCK_FILE_PATH: &str = "/tmp/router-lab.lock";
...
impl ServerSession {
    ...
    pub(crate) async fn new() -> Result<Self, RouterLabError> {
            ...
            .success()
        {
            ...
            return Err(RouterLabError::CannotObtainLock(user));
        }
    ...
    async fn create_all_folders(&self) -> Result<(), SshError> {
        ...
        for folder in [
            ...
            &CONFIG.server.traffic_monitor_pcap_path,
        ]
        .into_iter()
        .map(|p| {
