...
use bgpsim::{
    ...
    ospf::OspfImpl,
    prelude::{EventQueue, NetworkFormatter},
...
impl<'n, P: Prefix, Q: EventQueue<P> + Clone, Ospf: OspfImpl, S> RouterLab<'n, P, Q, Ospf, S> {
    ...
    pub fn generate_tofino_controller(&mut self) -> Result<String, RouterLabError> {
        ...
        let delay_port = CONFIG.server.delayer_tofino_base_port;
        ...
        for ((a, b), delay) in self.link_delays.iter() {
            if let (Ok(a_idx), Ok(b_idx)) = (
                ...
                if let (Some(a_port), Some(b_port)) = (
                    ...
                    let delay = (*delay as i32 + CONFIG.server.delayer_loop_offset as i32).max(0);
                    if delay > 0 {
                        writeln!(
                            &mut delays_rules,
                            "    {a_port: >3}: {{ {DELAY_ADDRS}, 'delay': {delay}, 'base_delay_port': {delay_port}, 'receiver_port': {b_port} }},",
                        )?;
                    }
                }
            }
...
impl<'n, P: Prefix, Q: EventQueue<P> + Clone, Ospf: OspfImpl> RouterLab<'n, P, Q, Ospf, Active> {
    ...
    pub(crate) async fn configure_tofino(&mut self) -> Result<(), RouterLabError> {
    ...
    pub async fn enable_link(&self, a: RouterId, b: RouterId) -> Result<(), RouterLabError> {
        self.state
            .tofino
    ...
    pub async fn disable_link(&self, a: RouterId, b: RouterId) -> Result<(), RouterLabError> {
        self.state
            .tofino
    ...
    fn find_link_tofino_ports(
        ...
        a: RouterId,
        b: RouterId,
    ) -> Result<Vec<&'static str>, RouterLabError> {
