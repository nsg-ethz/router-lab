...
impl SshSession {
    ...
    pub(crate) fn raw_command(&self, args: &[impl AsRef<OsStr>]) -> Command {
        let mut cmd = Command::from(std_command(self.name(), args));
...
/// - `args` as given by the other arguments.
/// - `destination` to connect to the given destination (or `none` if the path must exist).
pub(crate) fn std_command(name: &str, args: &[impl AsRef<OsStr>]) -> StdCommand {
    let mut cmd = StdCommand::new("ssh");
    cmd.arg("-oControlMaster=auto")
        .arg(format!("-oControlPath={}", control_path(name)))
        .arg("-oControlPersist=30m")
        .arg("-oBatchMode=yes")
        .arg("-oForwardX11=no")
        .arg("-oForwardX11Trusted=no")
        .args(args)
        .arg(name);
    cmd
}
...
/// - `oControlPersist=30m`
/// - `oBatchMode=yes`
pub(crate) fn scp_cmd(name: &str) -> StdCommand {
...
/// Return the path to the Control file (i.e., `ControlPath`)
pub(crate) fn control_path(name: &str) -> String {
    format!("/tmp/.router-lab-ssh-control-{name}")
}
