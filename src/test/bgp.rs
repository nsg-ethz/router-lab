...
#[test]
fn large_bgp_table() -> Result<(), Box<dyn std::error::Error>> {
    let table = "\
BGP routing table information for VRF default, address family IPv4 Unicast
BGP routing table entry for 20.0.9.0/24, version 237
...
Flags: (0x08001a) (high32 00000000) on xmit-list, is in urib, is best urib route, is in HW
  ...
  AS-Path: NONE, path sourced internal to AS
    1.0.3.1 (metric 2) from 1.0.3.1 (1.0.3.1)
      Origin IGP, MED not set, localpref 1, weight 0

  Advertised path-id 1
  Path type: internal, path is valid, is best path, no labeled nexthop, in rib
  AS-Path: NONE, path sourced internal to AS
    1.0.0.1 (metric 2) from 1.0.0.1 (1.0.0.1)
      Origin IGP, MED not set, localpref 180, weight 0

  Path type: internal, path is valid, not best reason: Local Preference, no labeled nexthop
  AS-Path: NONE, path sourced internal to AS
    1.0.1.1 (metric 2) from 1.0.1.1 (1.0.1.1)
      Origin IGP, MED not set, localpref 170, weight 0
...
";
    ...
    assert_eq!(
        routes,
        btreemap! {
            "20.0.9.0/24".parse()? => vec![
                BgpRoute {
                    net: "20.0.9.0/24".parse()?,
                    next_hop: "1.0.3.1".parse()?,
                    med: None,
                    local_pref: Some(1),
                    weight: 0,
                ...
                BgpRoute {
                    ...
                    path: vec![],
                    communities: btreeset![],
                    neighbor: "1.0.0.1".parse()?,
                    neighbor_id: "1.0.0.1".parse()?,
                    valid: true,
                    selected: true,
                    path_type: BgpPathType::Internal,
                },
